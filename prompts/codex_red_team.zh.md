...
- 纯代码决策、无外部 blast、无 production 数据（solo vibe coding 默认情况 — AI 几小时重写代码，代码层面的可逆性 cost 极低）
- 所有关键假设都还待验证（benchmark 没跑、数据没到、合伙人没聊）→ 现在质疑早，先去拿数据

### 何时该开 Section B

- 决策有**外部 blast**（公开承诺 / 用户 / 合作方 / 招聘 — 任何影响你之外的人）
- 决策接触**已生成的 production 数据**
- 决策**累积锁定**（≥3 个下游决策会建立在它之上）
- 你和合伙人内部已强共识 → 容易 group-think，需要外部视角
- 智囊团内部高度一致 → 可疑，需要异源质疑
- 涉及大资源投入（钱 / 时间 / 团队精力）
- **用户已强烈倾向某方向 → 这恰恰是红队最该开的时候。** Council 的存在是为了 challenge，不是 validate。强信念**不是**跳过的理由，**是**触发的理由。
